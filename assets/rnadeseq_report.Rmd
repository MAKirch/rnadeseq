...
```{r test, echo=TRUE, message=TRUE, warning=TRUE}
print("DEBUG: NEW HEATMAP CODE RUNNING")
```
...
#BiocManager::install(params$species_library, lib=species_dir, force=T)
            ...
            if (nrow(df) <= 100 & nrow(df) > 0) {
                ...
                for (i in c(1:nrow(df))){
                    ...
                    if (!isProvided(params$citest) & nrow(mat)>1){
                        ...
                        width_png  <- max(800, ncol(mat) * 60)
                        height_png <- max(600, nrow(mat) * 20)
                        width_pdf = max(8, ncol(mat)*0.5)
                        height_pdf = max(6, nrow(mat)*0.2)
                        
                        fontsize_row <- ifelse(nrow(mat) > 50, 4,
                          ifelse(nrow(mat) > 30, 6, 8))
                        ...
                        tryCatch(
                            {
                                png(filename = paste0("enrichment_analysis", "/",fname, "/", "heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", make.names(pathway$term_id), ".png"), width = width_png, height = height_png)
                                pheatmap(mat = mat, annotation_col = metadata_save, main = paste("Heatmap of normalized expression values (z-scores) for genes in pathway ", pathway$short_name, "(",pathway$source,")",sep=" "), scale = "row", cluster_cols = params$heatmaps_cluster_cols, cluster_rows = params$heatmaps_cluster_rows, fontsize_row = fontsize_row, show_rownames = show_rownames)
                                dev.off()
                            },
                            error=function(e) {
                        ...
                        tryCatch(
                            {
                                pdf(paste0("enrichment_analysis", "/", fname, "/", "heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", make.names(pathway$term_id), ".pdf"), width = width_pdf, height = height_pdf)
                                pheatmap(mat = mat, annotation_col = metadata_save, main = paste("Heatmap of normalized expression values (z-scores) for genes in pathway ", pathway$short_name, "(",pathway$source,")",sep=" "), scale = "row", cluster_cols = params$heatmaps_cluster_cols, cluster_rows = params$heatmaps_cluster_rows, fontsize_row = fontsize_row, show_rownames = show_rownames)
                                dev.off()
                            },
                            error=function(e) {
                        ...
                        tryCatch(
                            {
                                svg(paste0("enrichment_analysis", "/", fname, "/", "heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", make.names(pathway$term_id), ".svg"), width = width_pdf, height = height_pdf)
                                pheatmap(mat = mat, annotation_col = metadata_save, main = paste("Heatmap of normalized expression values (z-scores) for genes in pathway ", pathway$short_name, "(",pathway$source,")",sep=" "), scale = "row", cluster_cols = params$heatmaps_cluster_cols, cluster_rows = params$heatmaps_cluster_rows, fontsize_row = fontsize_row, show_rownames = show_rownames)
                                dev.off()
                            },
                            error=function(e) {
...
if (isProvided(params$path_genelist)){
    ...
    if (nrow(mat)>1){
        ...
        png(filename = paste0(genelist_heatmaps_dir, "/", "Heatmap_normalized_counts_gene_list.png"), width = width_png, height = height_png)
        pheatmap(mat = mat, annotation_col = metadata_save, main = "Genelist-specific Heatmap of normalized expression values (z-scores) for genes", scale = "row", cluster_cols = params$heatmaps_cluster_cols, cluster_rows = params$heatmaps_cluster_rows, fontsize_row = fontsize_row, show_rownames = show_rownames)
        dev.off()
        pdf(paste0(genelist_heatmaps_dir, "/", "Heatmap_normalized_counts_gene_list.pdf"), width = width_pdf, height = height_pdf)
        pheatmap(mat = mat, annotation_col = metadata_save, main = "Genelist-specific Heatmap of normalized expression values (z-scores) for genes", scale = "row", cluster_cols = params$heatmaps_cluster_cols, cluster_rows = params$heatmaps_cluster_rows, fontsize_row = fontsize_row, show_rownames = show_rownames)
        dev.off()
        svg(paste0(genelist_heatmaps_dir, "/", "Heatmap_normalized_counts_gene_list.svg"), width = width_pdf, height = height_pdf)
        pheatmap(mat = mat, annotation_col = metadata_save, main = "Genelist-specific Heatmap of normalized expression values (z-scores) for genes", scale = "row", cluster_cols = params$heatmaps_cluster_cols, cluster_rows = params$heatmaps_cluster_rows, fontsize_row = fontsize_row, show_rownames = show_rownames)
        dev.off()
    }
}
